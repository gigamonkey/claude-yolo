# yolo.py — Architecture & internals

This is the deep-dive reference for how `yolo.py` works, feature by feature.
It is intentionally **not** loaded into every session (that is what the lean
`CLAUDE.md` is for) — read the relevant section here when you need the internals
of a specific subsystem. Much of the rationale is historical; when in doubt,
trust the code over this prose.
...
reported multi-day revocation lag — claude-code issues
#34198/#48373/#59378/#43801), yolo does its own bookkeeping:
...
- **`wip`** — a tmux-resident **dashboard** for managing everything yolo, a
  ...
  **worktrees** (the `_worktree_rows` extracted from `do_list` — *every* worktree,
  ...
  `base` — but *not* `list`'s DIRECTORY column: the worktree dir is derived
  from repo + topic, and it was the one unbounded-width cell in the table),
  and **projects** (`_wip_projects`: the
  ...
  **REPO / DIRECTORY** table (the repo basename + the `~`-relative path), every
  ...
  not shown. This registry is kept **separate from `projects.json`
  ...
  its last-activity timestamp), in which case a three-way ask — dated with that
  activity ("last active 2d ago") — picks: `y` spawns `resume <topic>` to
  reconnect, `n` spawns `resume <topic> --new` (the topic's worktree, fresh
  Claude session), Esc/anything else cancels; remaining
  ...
  **Coloring** is full "angry fruit salad", applied only in the draw layer (so the
  ...
  REPO blue / DIRECTORY grey (uniform). `_format_table` measures **visible** width (`_visible_len` strips
...
## Test suite — file-by-file coverage
...
empty topic; a topic with history asking three ways — dated with its last
activity — `y` spawning `resume <topic>`, `n` spawning `resume <topic> --new`,
Esc cancelling), Enter on the `+` row prompting a directory then spawning `start --no-tmux`
