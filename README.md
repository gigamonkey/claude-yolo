...
to the pane's scrollback. Image builds in these windows use docker's plain
progress output (instead of the collapsing progress display) so the full build
log — warnings included — makes it into the capture. The launch prints the log's path (it lives in the
